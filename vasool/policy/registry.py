"""The fifteen, and how they resolve into one decision.
...
Running every guard fixes that and buys three things:
...
The order below is the spec's, with the two mandate guards of 2026-09-15
placed beside their kin, and kept because it reads well in a receipt: cheapest
and most absolute first, escalations last. Nothing depends on it.
"""
...
GUARD_CHAIN: tuple[Guard, ...] = (
    ...
    ConsentGuard(),           # DPDP
    MandateStateGuard(),      # a debit needs a live mandate
    ...
    AutopayPeakHoursGuard(),  # NPCI: no UPI Autopay execution at peak
    PreDebitNoticeGuard(),    # RBI 24h notice
    AFAThresholdGuard(),      # ₹15,000, or ₹1,00,000 by category
